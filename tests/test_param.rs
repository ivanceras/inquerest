...
fn test_equations_filter_groupby_having_orderby(){
    assert_eq!(
        ...
        , params("age=lt.13&student=eq.true|gender=eq.M&group_by=sum(age),grade,gender&having=min(age)=gt.13&order_by=age.desc,height.asc&x=123&y=456"));
}
...
fn test_equations_filter_groupby_having_orderby_page(){
    assert_eq!(
        ...
        , params("age=lt.13&student=eq.true|gender=eq.M&group_by=sum(age),grade,gender&having=min(age)=gt.13&order_by=age.desc,height.asc&page=20&page_size=100&x=123&y=456"));
}
